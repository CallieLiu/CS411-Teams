...
def test_create_meal(mock_cursor):
   ...
   # Call the function to create a new meal
    ...
    expected_arguments = ('Pizza', 'Italian', 5, 'MED')
...
def test_create_meal_invalid_price():
    """Test error when trying to create meal with an invalid price (e.g., negative price)."""
    ...
    with pytest.raises(ValueError, match="Invalid price: -180. Price must be a positive number."):
        create_meal(meal='Hamburger', cuisine='American', price=-180, difficulty='LOW')

    # Attempt to create a meal with a non-integer or non-float price
    with pytest.raises(ValueError, match="Invalid price: invalid. Price must be a positive number."):
        create_meal(meal='Burger', cuisine='American', price='invalid', difficulty='LOW')
...
def test_get_meal_by_id_deleted(mock_cursor,sample_meal1):
    mock_cursor.fetchone.return_value = [sample_meal1.id,sample_meal1.meal,sample_meal1.cuisine, sample_meal1.price, sample_meal1.difficulty,True]
    with pytest.raises(ValueError, match= "Meal with ID 1 has been deleted" ):
        get_meal_by_id(1)
...
def test_get_meal_by_id(mock_cursor,sample_meal1):
    ...
    expect_select_args_1=(1,)
    actual_select_args_1=mock_cursor.execute.call_args_list[0][0][1]
    actual_select_args=normalize_whitespace(mock_cursor.execute.call_args_list[0][0][0])
    assert expect_select_args==actual_select_args, f'expected select args is {expect_select_args}, but got {actual_select_args}'
...
def test_get_meal_by_name_existing(mock_cursor):
    # Define what the cursor should return for an existing meal
    mock_cursor.fetchone.return_value = (1, "Taco", "Mexican", 15, "MED", 0)
